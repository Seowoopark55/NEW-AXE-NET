...
/**
 * 신규 멤버의 첫 가입만 기존 AXE NET 회원가입 서버를 1회 사용합니다.
 * 이렇게 하면 아직 credential을 이관하지 않은 기존 멤버의 닉네임을
 * 제3자가 단순히 알고 있다는 이유만으로 새 비밀번호를 덮어쓸 수 없습니다.
 *
 * 가입 성공 후에는 즉시 new_axe_net.member_credentials로 bcrypt 이관되므로
 * 이후 로그인은 기존 서버를 거치지 않습니다.
 */
export async function legacySignup(nickname, password) {
  const apiUrl = process.env.AXE_LEGACY_API_URL || DEFAULT_LEGACY_API_URL;
  const response = await fetch(apiUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
  ...
  if (!response.ok) {
    const error = new Error(`기존 회원가입 서버 응답 오류 (${response.status})`);
    error.statusCode = 503;
    throw error;
  }
  ...
  try {
    ...
    const error = new Error('기존 회원가입 서버 응답을 해석할 수 없습니다.');
    error.statusCode = 503;
    throw error;
  }
  ...
  if (!data || data.result !== 'success' || !data.user) {
    ...
    const error = new Error(message);
    error.code = 'LEGACY_SIGNUP_FAILED';
    error.statusCode = /이미|사용 중|사용중/.test(message) ? 409 : 400;
    throw error;
  }

...
- 좌표가 근접한 9199↔9198, 8162↔8164, 7256↔7263, 3062↔3063, 3034↔3033도 같은 지역 이미지로 매칭했습니다.
- 페리코는 `cayo.png`를 브리핑 메인맵으로 연결했습니다.
- 4000/2060/1085 이미지는 교도소/휴메인/칠리아드 공략 메인맵에도 재사용합니다.
- SQL 변경 없음. DB의 기존 `map_001.png`~`map_017.png` 참조를 그대로 사용합니다.
...
## 로그인 자동완성 교차 주입 보정
- Chrome 비밀번호 관리자가 같은 도메인의 멤버/관리자 계정을 함께 보여주는 동작은 유지합니다.
- 다만 관리자 로그인 이메일 칸에 일반 멤버 닉네임이 자동 주입되면 이메일+비밀번호를 비웁니다.
- 멤버 로그인 닉네임 칸에 관리자 이메일이 자동 주입되면 닉네임+비밀번호를 비웁니다.
- `:-webkit-autofill` 상태에서만 보정하므로 사용자가 직접 입력하는 값은 건드리지 않습니다.
- Supabase/API/세션/SQL 변경은 없습니다.

## 적용
1. 기존 NEW AXE NET 프로젝트에 전체 덮어쓰기
2. GitHub Commit / Push
3. Vercel 자동 배포 확인
4. 멤버 로그인 ↔ 관리자 인증 탭을 각각 열어 자동 주입 여부 확인

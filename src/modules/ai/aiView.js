  ...
  root.innerHTML = `
    <section class="ops-ai">
      <header class="ops-ai__head">
        <div><span class="ops-ai__eyebrow">AXE INTELLIGENCE</span><h1>AXE AI 관리</h1><p>Discord 질문에 사용하는 지식과 학습 대기 항목을 관리합니다.</p></div>
    ...
    <div class="ops-ai-dashboard">
      ...
      <div class="ops-ai-dashboard-grid">
        ...
        <section class="ops-ai-panel">
          ...
          <div class="ops-ai-mini-list">${recent.length ? recent.map((item)=>`<div><strong>${escapeHtml(item.question)}</strong><span>${routeLabel(item.search_mode)} · ${escapeHtml(item.parsed_domain||'general')}</span><time>${fmtDate(item.created_at)}</time></div>`).join('') : '<p>질문 로그가 없습니다.</p>'}</div>
        </section>
      </div>
    </div>`;
}
  ...
  return `
    <section class="ops-ai-panel ops-ai-panel--table">
      <div class="ops-ai-toolbar">
        ...
        <select data-ai-filter="knowledgeDomain"><option value="all">전체 domain</option>${domains.map((v)=>`<option value="${attr(v)}" ${v===domain?'selected':''}>${escapeHtml(v)}</option>`).join('')}</select>
        <span>${rows.length}개 표시</span>
      </div>
      <div class="ops-ai-table-wrap"><table class="ops-ai-table"><thead><tr><th>지식</th><th>domain</th><th>별칭</th><th>상태</th><th>embedding</th><th>수정일</th><th></th></tr></thead><tbody>
...
function knowledgeRow(item){return `<tr>
  ...
  <td><code>${escapeHtml(item.domain)}</code>${item.category?`<small>${escapeHtml(item.category)}</small>`:''}</td>
  ...
  <td><span class="ops-ai-status is-${attr(item.embedding_status)}">${escapeHtml(item.embedding_status||'-')}</span></td>
...
function renderUnknown(ai){
  ...
  return `<section class="ops-ai-panel ops-ai-panel--table">
    ...
    <div class="ops-ai-table-wrap"><table class="ops-ai-table ops-ai-table--unknown"><thead><tr><th>질문</th><th>domain</th><th>횟수</th><th>최근 질문</th><th>상태</th><th></th></tr></thead><tbody>
    ${rows.length?rows.map(unknownRow).join(''):'<tr><td colspan="6" class="ops-ai-empty">해당 상태의 질문이 없습니다.</td></tr>'}</tbody></table></div>
  </section>`;
}
function unknownRow(item){return `<tr><td><strong>${escapeHtml(item.question)}</strong>${item.admin_note?`<span>${escapeHtml(item.admin_note)}</span>`:''}</td><td><code>${escapeHtml(item.domain||'general')}</code></td><td><b>${num(item.ask_count)}</b></td><td><time>${fmtDate(item.last_asked_at)}</time></td><td><span class="ops-ai-status is-${attr(item.status)}">${escapeHtml(item.status)}</span></td><td><div class="ops-ai-row-actions">${item.status==='open'?`<button type="button" class="is-primary" data-ai-learn="${item.id}">지식으로 등록</button><button type="button" data-ai-ignore="${item.id}">무시</button>`:`<button type="button" data-ai-reopen="${item.id}">다시 열기</button>`}</div></td></tr>`;}
...
function logRow(item){return `<tr><td><strong>${escapeHtml(item.question)}</strong><span>${escapeHtml(item.resolved_query||'')}</span></td><td><code>${escapeHtml(item.parsed_domain||'-')}</code><small>${escapeHtml(item.parsed_intent||'-')}</small></td><td><span class="ops-ai-route-pill is-${routeClass(item.search_mode)}">${routeLabel(item.search_mode)}</span></td><td><span class="ops-ai-status is-${attr(item.result_status)}">${escapeHtml(item.result_status)}</span><small>${num(item.match_count)}건</small></td><td>${item.confidence==null?'-':pct(item.confidence)}</td><td>${item.duration_ms==null?'-':`${num(item.duration_ms)}ms`}</td><td><time>${fmtDate(item.created_at)}</time></td></tr>`;}
...
function renderEditor(ai){
  ...
  return `<div class="ops-ai-modal"><div class="ops-ai-modal__backdrop" data-ai-close></div><section class="ops-ai-editor" role="dialog" aria-modal="true"><header><div><span>${unknown?'LEARN FROM QUESTION':item?'EDIT KNOWLEDGE':'NEW KNOWLEDGE'}</span><h2>${unknown?'미답변을 지식으로 등록':item?'AI 지식 수정':'AI 지식 등록'}</h2>${unknown?`<p>질문: ${escapeHtml(unknown.question)} · ${num(unknown.ask_count)}회</p>`:'<p>저장 즉시 AXE AI 검색 카탈로그에 반영됩니다.</p>'}</div><button type="button" data-ai-close>×</button></header><form data-ai-form><div class="ops-ai-form-grid">
    <label><span>Domain *</span><input name="domain" value="${attr(defaults.domain)}" required placeholder="processing" /></label>
    ...
    <label><span>Source type</span><select name="source_type">${['manual','info','fund','member','asset','outlaw','notice','rules','map','community','legacy'].map((v)=>`<option value="${v}" ${v===defaults.source_type?'selected':''}>${v}</option>`).join('')}</select></label>
    <label><span>우선순위</span><input name="priority" type="number" min="-100" max="100" value="${attr(defaults.priority)}" /></label>
    <label><span>Source table</span><input name="source_table" value="${attr(defaults.source_table)}" placeholder="선택" /></label>
    <label><span>Source key</span><input name="source_key" value="${attr(defaults.source_key)}" placeholder="고유 키 · 선택" /></label>
